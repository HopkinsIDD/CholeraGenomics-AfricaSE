...
new_seq <- read_xlsx("./supp_data/supp_data_1_sample_metadata.xlsx") %>% filter(meeting_criteria)
...
annotation_raw <- read_xlsx("./supp_data/supp_data_2_annotation.xlsx")
...
subsample_bysample_depth <-
  read_xlsx("./supp_data/supp_data_4_subsample_sampletype.xlsx") 
...
subsample_pos_cov_binned <- 
  read_xlsx("./supp_data/supp_data_5_subsample_poscov.xlsx") 
...
# read ML tree
tree_1488 <- read.tree("./supp_data/supp_data_6_MLtree.treefile")
...
### Supp Figure 3: Maximum likelihood tree of 1488 V. cholerae genomes. Colored by country

```{r supp fig3, echo=F, message=F, warning=F, fig.height=8, fig.width=12, background="white"}


##supplemental fig3A
supp_fig3 <- ggtree(tree_1488,
                      layout="circular",
                      size=0.5
                      ) %<+% anno_withnode +
  geom_treescale(x=0.003,
...
T13_tMRCA <- read_xlsx("./supp_data/supp_data_7_tMRCAT13_data.xlsx") %>%
...
TZA_2015_case <- read_xlsx("./supp_data/supp_data_3_cholera_cases.xlsx", sheet = "TZA") %>%
  dplyr::select(NAME_1,cases15) %>%
  rename(case_n = cases15) %>%
...
TZA_2017_case <- read_xlsx("./supp_data/supp_data_3_cholera_cases.xlsx", sheet = "TZA") %>%
...
TZA_2018_case <- read_xlsx("./supp_data/supp_data_3_cholera_cases.xlsx", sheet = "TZA") %>%
...
UGA_2015_case <- read_xlsx("./supp_data/supp_data_3_cholera_cases.xlsx", sheet = "UGA") %>%
  dplyr::select(NAME_1,cases15) %>%
  rename(case_n = cases15) %>%
  mutate(case_cat = factor(case_when(case_n == 0 ~ 1,
                                      case_n > 0 & case_n <= 10 ~ 2,
...
UGA_2018_case <- read_xlsx("./supp_data/supp_data_3_cholera_cases.xlsx", sheet = "UGA") %>%
  dplyr::select(NAME_1,cases18) %>%
  rename(case_n = cases18) %>%
  mutate(case_cat = factor(case_when(case_n == 0 ~ 1,
                                      case_n > 0 & case_n <= 10 ~ 2,
...
ZMB_2016_case <- read_xlsx("./supp_data/supp_data_3_cholera_cases.xlsx", sheet = "ZMB") %>%
  dplyr::select(NAME_1,cases16) %>%
  rename(case_n = cases16) %>%
  mutate(case_cat = factor(case_when(case_n == 0 ~ 1,
                                      case_n > 0 & case_n <= 10 ~ 2,
