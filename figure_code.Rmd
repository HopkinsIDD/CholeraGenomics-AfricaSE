...
## wet land map downloaded from: https://files.worldwildlife.org/wwfcmsprod/files/Publication/file/8ark3lcpfw_GLWD_level1.zip?_ga=2.269024947.35138699.1698458757-1278031685.1698458757
###download the maps into the maps folder
...
new_seq <- read_xlsx("./supp_data/supplementary_appendix_3.xlsx") %>% filter(meeting_criteria)
...
annotation_raw <- read_xlsx("./supp_data/supplementary_appendix_2.xlsx")
...
sequenced_new <- new_seq %>% filter(meeting_criteria) %>%
            filter(Year != "unknown") %>%
            mutate(year = as.numeric(Year)) %>%
            mutate(category=2)%>%
            dplyr::select(SampleID,year,ISO3,category) %>%
...
# read supplemental 2 data
case_data <- read_xlsx("./supp_data/supplementary_appendix_4.xlsx",
                       sheet="country_level") %>% 
             rename(year=Year) %>%
...
fig1_panelB <- ggplot() + geom_point(data=case_data_pad,
  ...
  scale_fill_manual(name="Country",
                    values=c("Uganda"="#76CCAE",
                             "Kenya"="#BE91C2",
                             "Tanzania"="#F5D58D",
                             "Malawi"="#F6998E",
                             "Zambia"="#C0D9E9")
                    )  +
  scale_alpha_manual(name="Category",
                     values=c(0.5,1)) +
...
subsample_bysample_depth <-
  read_xlsx("./supp_data/supplementary_appendix_5.xlsx") 


## outlier statistics
subsample_bysample_depth_stats <- subsample_bysample_depth %>%
  group_by(SampleType) %>%
  summarise(low_outlier=quantile(median_depth,0.25) - 1.5*IQR(median_depth),
            high_outlier=quantile(median_depth,0.75) + 1.5*IQR(median_depth)) %>%
  ungroup()
...
fig1_panelC <- subsample_bysample_depth %>% 
  ...
  mutate(SampleType=factor(case_when(
         ...
         include_ML=factor(as.numeric(include),labels=c("No","Yes"))
         ) %>%
  ggplot() +
  geom_jitter(aes(x=SampleType, y=median_depth, shape=include_ML),
...
subsample_pos_cov_binned <- 
  read_xlsx("./supp_data/supplementary_appendix_6.xlsx") 
...
# read ML tree
tree_1488 <- read.tree("./supp_data/supplementary_appendix_7.treefile")

# obtain the nodes of the tree
node <- as_tibble(tree_1488) %>% mutate(label=gsub("'",'',label),
                                        label=gsub("_","/",label))
...
teyear_visdata <- merge(teyear_distribution,
                        ...
                        by="ISO3",
                        all.x=T) %>%
                  mutate(x_dodge=case_when(te %in% c("AFR8","AFR10") ~ 0,
                                           te %in% c("AFR11","AFR13") ~ 0.1
                                           ),
                         y_dodge=case_when(te %in% c("AFR8","AFR11") ~ 0.2,
                                           te %in% c("AFR10","AFR13") ~ 0.3
                                           ),
...
### Figure 3A: Maximum likelihood tree of 1488 V. cholerae genomes. Colored tips: 114 genomes generated in this study, colored by country of sample collection. Unfilled tips: 1374 previously published genomes after excluding 4 genomes generatede in this study with no date and 11 outliers detected by TempEst 

```{r fig3A, echo=F, message=F, warning=F, fig.height=8, fig.width=8, background="white"}


tree_vis <- ggtree(tree_1488, 
                   layout="circular", 
                   size=0.5
                   ) %<+% anno_withnode +
...
tree_dt <- tree_dt[isTip == TRUE&te!="" & 
                   te != "sporadic/local outbreak"][order(y)]
...
coord_groups1[te == "AFR11",]$y_mid <- 1140
coord_groups1[te == "AFR12",]$y_mid <- 1260
coord_groups1[te == "AFR12",]$hjust_adj <- 0
coord_groups1[te == "AFR16",]$y_mid <- 1200
coord_groups1[te == "AFR17",]$y_mid <- 1100
...
anno_withnode_zoom <- anno_withnode %>% 
        mutate(ISO3_zoom=ifelse(status == "Genomes sequenced in this paper"|
                                taxa %in% c("ERR114418", "ERR117600")| # previously sporadic outbreak (now AFR16,17)
                                te %in% c("AFR10","AFR11","AFR13"), 
                                paste(ISO3,"|",year,sep = ""),
                                ISO3
                                ),
               ISO3_fig = ifelse(status == "Genomes sequenced in this paper",
                            ISO3, "Other")
               )


fig3_panelBD <- ggtree(tree_1488,
                       layout="circular",
                       size=0.2
                       ) %<+% anno_withnode_zoom +
  geom_treescale(x=0.003,
  ...
  geom_tippoint(aes(color = ISO3_fig,
                    size = status,
                    shape = status))  +
  scale_color_manual(name="",
                     values=c("UGA"="#528F78",
  ...
  geom_tiplab(aes(label = ISO3_zoom),size = 0.8)
...
# # zoom-in tree data
# anno_withnode_zoom_inv <- anno_withnode %>% 
#         mutate(ISO3_zoom=paste(ISO3,"|",year,sep = ""),
#                ISO3_fig = ifelse(status == "Genomes sequenced in this paper",
#                             ISO3, "Other")
#                )
# 
# 
# fig3_panelBD_inv <- ggtree(tree_1488,
#                        layout="circular",
#                        size=0.2
#                        ) %<+% anno_withnode_zoom_inv +
#   geom_treescale(x=0.003,
#                  y=760,
#                  fontsize=4,
#                  linesize=1, 
...
#   scale_shape_manual(name = "", values = c(16,1)) +
#   geom_tiplab(aes(label = ISO3_zoom),size = 0.8)
# print(fig3_panelBD_inv)
# ggsave("fig3_panelBD_inv.svg", width=32, height=32, units="in")

```

```{r afr13 kenya seq, include=F}

AFR13_KEN_earlyseq <- anno_withnode %>% filter(te == "AFR13",
                                               ISO3 == "KEN",
                                               status == "Genomes sequenced in this paper",
                                               year <= 2013)

AFR13_KEN_earlyseq_quality <- new_seq %>% filter(accession %in% AFR13_KEN_earlyseq$taxa)

```


### zoom view for wave-colored 

# ```{r zoom_by_wave, echo=F, message=F, warning=F, fig.height=32, fig.width=32, background="white"}
# 
# # zoom-in tree data
# anno_withnode_zoom <- anno_withnode %>% 
#         mutate(ISO3_zoom=ifelse(status == "Genomes sequenced in this paper",
...
#                size=0.8) +
#   geom_tippoint(aes(color = factor(wave),
#                     size = status,
#                     shape = status)) +
#   scale_size_manual(name = "", values = c(1,0.6)) + 
#   scale_shape_manual(name = "", values = c(16,1)) +
#   geom_tiplab(aes(label = taxa),size = 0.8)
#  
# 
# print(wave_zoom)
# 
...
### Zoom view of all lineage

# ```{r zoom_all_label, echo=F, message=F, warning=F, fig.height=32, fig.width=32, background="white"}
# 
# # zoom-in tree data
# anno_withnode_zoom_all <- anno_withnode %>% 
#         mutate(ISO3_zoom=ifelse(continent=="Africa",
#                                 paste(ISO3,"|",year,sep = ""),
#                                 ISO3
#                                 ),
#                ISO3_zoom=ifelse(!is.na(district),
#                                 paste(ISO3_zoom,"|",district,sep = ""),
#                                 ISO3_zoom
#                                 ),
#                ISO3_fig = ifelse(status == "Genomes sequenced in this paper",
#                             ISO3, "Other")
...
# 
# 
# fig_zoomall <- ggtree(tree_1488,
#                        layout="circular",
#                        size=0.2
#                        ) %<+% anno_withnode_zoom_all +
#   geom_treescale(x=0.003,
#                  y=760,
#                  fontsize=4,
#                  linesize=1, 
...
T13_tMRCA <- read_xlsx("./supp_data/supplementary_appendix_8.xlsx") %>%
...
color_cocir <- c("AFR10"="#FFD915",
                 "AFR11"="#0F9543",
                 "AFR13"="#00A3EE",
                 "AFR10,AFR13"="darkgreen",
                 "water"="#4e92eb")




color_case <- c("0"="#f1f1f1",
                "1-10"="#cbcdcf",
                "11-100"="#9e9fa5",
                "101-"="#76777d",
                "water"="#4e92eb")
...
### Figure4A: Co-circulation of multiple lineages at country or sub-country level (Malawi: 2015)
...
MWI_2015_case <- read_xlsx("./supp_data/supplementary_appendix_4.xlsx",
                           sheet="MWI") %>%
...
fig4_panelA <- ggarrange(fig4_MWI_2015_te,
                         fig4_MWI_2015_case,
                         nrow = 1
                         )
...
### Figure4B: Co-circulation of multiple lineages at country or sub-country level (Malawi: 2016)
...
MWI_2016_case <- read_xlsx("./supp_data/supplementary_appendix_4.xlsx",
                           sheet="MWI") %>%
...
fig4_panelB <- ggarrange(fig4_MWI_2016_te,
                         fig4_MWI_2016_case,
                         nrow = 1
                         )
...
### SuppFigure1A: The root-to-tip regression visualization of all 1499 genomes

```{r supp_fig1A, echo=F, message=F, warning=F, fig.height=8, fig.width=8, background="white"}

vc_1488_rtt <- 
  read_xlsx("./supp_data/supplementary_appendix_9.xlsx",
             sheet = "full_tree")

vc_1488_rtt_model <- lm(distance ~ date, data=vc_1488_rtt)

vc_1488_rtt_xintercept <- -coef(vc_1488_rtt_model)[1]/coef(vc_1488_rtt_model)[2]


suppfig1_panelA <- vc_1488_rtt %>% ggplot() + 
                          aes(x=date, y=distance) +
                          geom_point(aes(color=group)) +
                          geom_abline(intercept=coef(vc_1488_rtt_model)[1],
                                      slope=coef(vc_1488_rtt_model)[2],
                                      color="black")+
                          ...
                          stat_poly_eq(aes(label = after_stat(adj.rr.label))) + 
                          coord_cartesian(xlim=c(vc_1488_rtt_xintercept - 5,
                                                 max(vc_1488_rtt$date) + 3),
                                          ylim=c(0, max(vc_1488_rtt$distance)))+
                          scale_x_continuous(breaks = c(1910.402,1960,1980,2000,2020),
                                             labels=c(1910,1960,1980,2000,2020)) +
...
### SuppFigure1B: The root-to-tip regression visualization of 960 wave 3 genomes that were included in the BEAST analysis

```{r supp_fig1B, echo=F, message=F, warning=F, fig.height=8, fig.width=8, background="white"}

vc_wave3_rtt <- read_xlsx("./supp_data/supplementary_appendix_9.xlsx",
                         sheet="wave3_tree")

vc_wave3_rtt_model <- lm(distance ~ date, data=vc_wave3_rtt)

vc_wave3_rtt_xintercept <- -coef(vc_wave3_rtt_model)[1]/coef(vc_wave3_rtt_model)[2]

suppfig1_panelB <- vc_wave3_rtt %>% ggplot() + 
                        aes(x=date,
                            y=distance) +
                        geom_point(color="#B3B5B5") +
                        geom_abline(intercept=coef(vc_wave3_rtt_model)[1],
                                    slope=coef(vc_wave3_rtt_model)[2],
                                    color="black")+
                        ylab("root-to-tip divergence") + 
                        xlab("time")  +
                        stat_poly_eq(aes(label = after_stat(adj.rr.label)))+ 
                          coord_cartesian(xlim=c(vc_wave3_rtt_xintercept - 5,
                                                 max(vc_wave3_rtt$date) + 3),
                                          ylim=c(0, max(vc_wave3_rtt$distance)))+
                          scale_x_continuous(breaks = c(1991.048,2000,2010,2020),
                                             labels=c(1991,2000,2010,2020)) +
                          scale_y_continuous(expand=c(0,0))+
                        theme_bw(base_size=18) +
                        theme(panel.border=element_blank(),
                              panel.grid.major=element_blank(),
...
### Other co-circulation

### KEN:2009

```{r co-circulation_KEN_2009, echo=F, message=F, warning=F, fig.height=8, fig.width=16, background="white"}
...
KEN_2009_case <- read_xlsx("./supp_data/supplementary_appendix_4.xlsx",
                           sheet="KEN") %>%
...
##
fig_KEN_2009_te <- KEN_2009_case_te %>% ggplot()  +
                            geom_sf(size=0.6,
                                    fill="white"
                                    ) +
                            geom_sf(size=0.6,
...
fig_KEN_2009_case <- KEN_2009_case_te %>% ggplot()  +
...
fig_KEN_2009_cocir <- ggarrange(fig_KEN_2009_te,
                         fig_KEN_2009_case,
                         nrow = 1
                         )
...
print(fig_KEN_2009_cocir)
```

### KEN:2010

```{r co-circulation_KEN_2010, echo=F, message=F, warning=F, fig.height=8, fig.width=16, background="white"}

# read Kenya 2010 case data
KEN_2010_case <- read_xlsx("./supp_data/supplementary_appendix_4.xlsx",
                           sheet="KEN") %>%
...
##
fig_KEN_2010_te <- KEN_2010_case_te %>% ggplot()  +
                            geom_sf(size=0.6,
                                    fill="white"
                                    ) +
                            geom_sf(size=0.6,
...
fig_KEN_2010_case <- KEN_2010_case_te %>% ggplot()  +
...
fig_KEN_2010_cocir <- ggarrange(fig_KEN_2010_te,
                         fig_KEN_2010_case,
                         nrow = 1
                         )
...
print(fig_KEN_2010_cocir)
```


### KEN:2011

```{r co-circulation_KEN_2011, echo=F, message=F, warning=F, fig.height=8, fig.width=16, background="white"}

# read Kenya 2011 case data
KEN_2011_case <- read_xlsx("./supp_data/supplementary_appendix_4.xlsx",
                           sheet="KEN") %>%
                  dplyr::select(NAME_1, cases10) %>%
                  rename(case_n=cases10) %>%
                  mutate(case_cat=factor(
...
# summarise Kenya 2011 te distribution
KEN_2011_te <- anno_withnode %>%
              filter(!stri_isempty(district) &
                     te != "sporadic/local outbreak" &
                     te != "AFR16" &
                     te != "AFR17"
                     ) %>%
              filter(ISO3 == "KEN" & year == 2011)

KEN_2011_te_sum <- KEN_2011_te %>% 
              group_by(district, te) %>%
              summarise(n=n()) %>% 
              ungroup() %>%
              mutate(label=paste(te, " : ", n, sep = ""))

KEN_2011_te_country <- KEN_2011_te_sum %>%
                      arrange(district,
                              as.numeric(str_replace(te,"AFR",""))
                              ) %>%
                      group_by(district) %>%
...
KEN_2011_case_te <- Reduce(function(x, y) {merge(x, y, by="NAME_1", all.x=T)},
                           list(KEN1, KEN_2011_case, KEN_2011_te_country))

# summarise te distribution of Kenya neighboring countries
KEN_2011_neighbor_te <- merge(KEN_neighbors,
                              anno_withnode %>%
                                filter(te != "sporadic/local outbreak" &
                                       te != "AFR16" &
                                       te != "AFR17") %>%
...
KEN_2011_neighbor_te$x = c(39, 42, 34, 35, 34)
KEN_2011_neighbor_te$y = c(4.5, 2, 5.5, -2.5, 2.3)


##
fig_KEN_2011_te <- KEN_2011_case_te %>% ggplot()  +
                            geom_sf(size=0.6,
                                    fill="white"
                                    ) +
                            geom_sf(size=0.6,
                                    aes(fill=te_sum),
                                    data=KEN_2011_case_te %>% 
                                          filter(!is.na(te_sum))
                                    ) +
                            geom_sf(data=water_Africa,
                                    aes(fill=type)
...
fig_KEN_2011_case <- KEN_2011_case_te %>% ggplot()  +
                            geom_sf(data=KEN_2011_case_te,
                                    aes(fill=case_cat),
                                    size=1
                                    ) +
                            geom_sf(data=water_Africa,
...
fig_KEN_2011_cocir <- ggarrange(fig_KEN_2011_te,
                         fig_KEN_2011_case,
                         nrow = 1
                         )
...
print(fig_KEN_2011_cocir)
```


<!-- ####TZA: 2015 -->

<!-- ```{r co-circulation_TZA_2015, echo=F, message=F, warning=F, fig.height=8, fig.width=16, background="white"} -->

<!-- TZA_neighbors <- data.frame(ISO3 = c("UGA","KEN","RWA","BDI", -->
<!--                                      "ZMB","MWI","MOZ","COD")) -->

<!-- ####TZA_2015 -->

<!-- TZA_2015_case <- read_xlsx("./supp_data/supplementary_appendix_4.xlsx", sheet = "TZA") %>% -->
<!--   dplyr::select(NAME_1,cases15) %>% -->
<!--   rename(case_n = cases15) %>% -->
<!--   mutate(case_cat = factor(case_when(case_n == 0 ~ 1, -->
<!--                                      case_n > 100 ~ 4), -->
<!--                            labels = c("0","101-"))) -->


<!-- TZA_2015_te <- anno_withnode %>% -->
<!--               filter(!stri_isempty(district) & -->
<!--                      te != "sporadic/local outbreak" & -->
<!--                      te != "AFR16" & -->
<!--                      te != "AFR17" -->
<!--                      ) %>% -->
<!--   filter(ISO3 == "TZA" & year == 2015) -->

<!-- TZA_2015_te_sum <- TZA_2015_te %>% group_by(district,te) %>% -->
<!--   summarise(n = n()) %>% ungroup() %>% -->
<!--   mutate(label = paste(te," : ",n,sep = "")) -->

<!-- TZA_2015_te_country <- TZA_2015_te_sum %>% -->
<!--   arrange(district,as.numeric(str_replace(te,"T",""))) %>% -->
<!--   group_by(district) %>% -->
<!--   summarise(te_sum = paste0(te,collapse = ","), -->
<!--             te_label = paste0(label,collapse = ", ")) %>% ungroup() %>% -->
<!--   rename(NAME_1 = district) %>% -->
<!--   mutate(te_label = str_wrap(te_label, width = 20)) -->

<!-- TZA_2015_case_te <- Reduce(function(x,y) {merge(x,y,by = "NAME_1",all.x = T)}, -->
<!--                            list(TZA1,TZA_2015_case,TZA_2015_te_country)) -->


<!-- TZA_2015_neighbor_te <- merge(TZA_neighbors, -->
<!--                               anno_withnode %>% -->
<!--                                 filter(te != "sporadic/local outbreak" & -->
<!--                                        te != "AFR16" & -->
<!--                                        te != "AFR17") %>% -->
<!--                                 filter(ISO3 %in% TZA_neighbors$ISO3 & -->
<!--                                          year == 2015)%>% group_by(ISO3,te) %>% -->
<!--                                 summarise(n = n()) %>% group_by(ISO3) %>% -->
<!--                                 summarise(neighbor_label = -->
<!--                                             paste0(te,collapse = ",")) %>% -->
<!--                                 ungroup() %>% mutate(neighbor_label = -->
<!--                                                        paste(ISO3," :\n",neighbor_label,sep = "")), -->
<!--                               by = "ISO3",all.x = T) %>% -->
<!--   mutate(neighbor_label = -->
<!--            ifelse(is.na(neighbor_label), -->
<!--                   paste(ISO3, " :\nNo data", sep = ""), -->
<!--                   neighbor_label)) -->
...
<!-- TZA_2015_neighbor_te$x = c(30,29,38,39,33.8,30,31,31) -->
<!-- TZA_2015_neighbor_te$y = c(-3.2,-6,-1.5,-12,-11.3,-1.8,0,-10) -->
<!-- TZA_2015_neighbor_te$size = c('s','l','l','l','s','s','l','l') -->

<!-- fig_TZA_2015_te <- TZA_2015_case_te %>% ggplot()  + -->
<!--                             geom_sf(size=0.6, -->
<!--                                     fill="white" -->
<!--                                     ) + -->
<!--                             geom_sf(size=0.6, -->
<!--                                     aes(fill=te_sum), -->
<!--                                     data=TZA_2015_case_te %>% -->
<!--                                           filter(!is.na(te_sum)) -->
<!--                                     ) + -->
<!--                             geom_sf(data=water_Africa, -->
<!--                                     aes(fill=type) -->
...
<!--                                           y=y -->
<!--                                           ), -->
<!--                                       data=TZA_2015_neighbor_te -->
<!--                                       ) + -->
<!--                             coord_sf(xlim = c(28,41), -->
<!--                                      ylim = c(-12,0)) + -->
<!--                             theme_map() + -->
<!--                             theme(legend.position="right") + -->
<!--                             scale_fill_manual(name="", -->
...
<!-- fig_TZA_2015_case <- TZA_2015_case_te %>% ggplot()   + -->
<!--                             geom_sf(data=TZA_2015_case_te, -->
<!--                                     aes(fill=case_cat), -->
<!--                                     size=1 -->
<!--                                     ) + -->
<!--                             geom_sf(data=water_Africa, -->
...
<!-- fig_TZA_2015_cocir <- ggarrange(fig_TZA_2015_te, -->
<!--                             fig_TZA_2015_case, -->
<!--                             nrow = 1 -->
<!--                             ) -->
...
<!-- print(fig_TZA_2015_cocir) -->
<!-- ``` -->


####TZA: 2017

```{r co-circulation_TZA_2017, echo=F, message=F, warning=F, fig.height=8, fig.width=16, background="white"}
...
TZA_2017_case <- read_xlsx("./supp_data/supplementary_appendix_4.xlsx", sheet = "TZA") %>%
  dplyr::select(NAME_1,cases17) %>%
  rename(case_n = cases17) %>%
  mutate(case_cat = factor(case_when(case_n == 0 ~ 1,
                                     case_n > 100 ~ 4),
                           labels = c("0","101-")))


TZA_2017_te <- anno_withnode %>%
              filter(!stri_isempty(district) &
                     te != "sporadic/local outbreak" &
                     te != "AFR16" &
                     te != "AFR17"
                     ) %>%
  filter(ISO3 == "TZA" & year == 2017)

TZA_2017_te_sum <- TZA_2017_te %>% group_by(district,te) %>%
  summarise(n = n()) %>% ungroup() %>%
  mutate(label = paste(te," : ",n,sep = ""))

TZA_2017_te_country <- TZA_2017_te_sum %>%
  arrange(district,as.numeric(str_replace(te,"T",""))) %>%
  group_by(district) %>%
  summarise(te_sum = paste0(te,collapse = ","),
            te_label = paste0(label,collapse = ", ")) %>% ungroup() %>%
...
TZA_2017_case_te <- Reduce(function(x,y) {merge(x,y,by = "NAME_1",all.x = T)},
                           list(TZA1,TZA_2017_case,TZA_2017_te_country))


TZA_2017_neighbor_te <- merge(TZA_neighbors,
                              anno_withnode %>%
                                filter(te != "sporadic/local outbreak" &
                                       te != "AFR16" &
                                       te != "AFR17") %>%
                                filter(ISO3 %in% TZA_neighbors$ISO3 &
                                         year == 2017)%>% group_by(ISO3,te) %>%
                                summarise(n = n()) %>% group_by(ISO3) %>%
                                summarise(neighbor_label =
                                            paste0(te,collapse = ",")) %>%
                                ungroup() %>% mutate(neighbor_label =
...
TZA_2017_neighbor_te$x = c(30,29,38,39,33.8,30,31,31)
TZA_2017_neighbor_te$y = c(-3.2,-6,-1.5,-12,-11.3,-1.8,0,-10)
TZA_2017_neighbor_te$size = c('s','l','l','l','s','s','l','l')

fig_TZA_2017_te <- TZA_2017_case_te %>% ggplot()  +
                            geom_sf(size=0.6,
                                    fill="white"
                                    ) +
                            geom_sf(size=0.6,
                                    aes(fill=te_sum),
                                    data=TZA_2017_case_te %>%
                                          filter(!is.na(te_sum))
                                    ) +
                            geom_sf(data=water_Africa,
                                    aes(fill=type)
...
fig_TZA_2017_case <- TZA_2017_case_te %>% ggplot()   +
                            geom_sf(data=TZA_2017_case_te,
                                    aes(fill=case_cat),
                                    size=1
                                    ) +
                            geom_sf(data=water_Africa,
...
fig_TZA_2017_cocir <- ggarrange(fig_TZA_2017_te,
                            fig_TZA_2017_case,
                            nrow = 1
                            )
...
print(fig_TZA_2017_cocir)
```

####TZA: 2018

```{r co-circulation_TZA_2018, echo=F, message=F, warning=F, fig.height=8, fig.width=16, background="white"}
...
TZA_2018_case <- read_xlsx("./supp_data/supplementary_appendix_4.xlsx", sheet = "TZA") %>%
  dplyr::select(NAME_1,cases18) %>%
  rename(case_n = cases18) %>%
  mutate(case_cat = factor(case_when(case_n == 0 ~ 1,
                                     case_n > 100 ~ 4),
                           labels = c("0","101-")))


TZA_2018_te <- anno_withnode %>%
              filter(!stri_isempty(district) &
                     te != "sporadic/local outbreak" &
                     te != "AFR16" &
                     te != "AFR17"
                     ) %>%
  filter(ISO3 == "TZA" & year == 2018)

TZA_2018_te_sum <- TZA_2018_te %>% group_by(district,te) %>%
  summarise(n = n()) %>% ungroup() %>%
  mutate(label = paste(te," : ",n,sep = ""))

TZA_2018_te_country <- TZA_2018_te_sum %>%
  arrange(district,as.numeric(str_replace(te,"T",""))) %>%
  group_by(district) %>%
  summarise(te_sum = paste0(te,collapse = ","),
            te_label = paste0(label,collapse = ", ")) %>% ungroup() %>%
...
TZA_2018_case_te <- Reduce(function(x,y) {merge(x,y,by = "NAME_1",all.x = T)},
                           list(TZA1,TZA_2018_case,TZA_2018_te_country))


TZA_2018_neighbor_te <- merge(TZA_neighbors,
                              anno_withnode %>%
                                filter(te != "sporadic/local outbreak" &
                                       te != "AFR16" &
                                       te != "AFR17") %>%
                                filter(ISO3 %in% TZA_neighbors$ISO3 &
                                         year == 2018)%>% group_by(ISO3,te) %>%
                                summarise(n = n()) %>% group_by(ISO3) %>%
                                summarise(neighbor_label =
                                            paste0(te,collapse = ",")) %>%
                                ungroup() %>% mutate(neighbor_label =
...
TZA_2018_neighbor_te$x = c(30,29,38,39,33.8,30,31,31)
TZA_2018_neighbor_te$y = c(-3.2,-6,-1.5,-12,-11.3,-1.8,0,-10)
TZA_2018_neighbor_te$size = c('s','l','l','l','s','s','l','l')

fig_TZA_2018_te <- TZA_2018_case_te %>% ggplot()  +
                            geom_sf(size=0.6,
                                    fill="white"
                                    ) +
                            geom_sf(size=0.6,
                                    aes(fill=te_sum),
                                    data=TZA_2018_case_te %>%
                                          filter(!is.na(te_sum))
                                    ) +
                            geom_sf(data=water_Africa,
                                    aes(fill=type)
...
fig_TZA_2018_case <- TZA_2018_case_te %>% ggplot()   +
                            geom_sf(data=TZA_2018_case_te,
                                    aes(fill=case_cat),
                                    size=1
                                    ) +
                            geom_sf(data=water_Africa,
...
fig_TZA_2018_cocir <- ggarrange(fig_TZA_2018_te,
                            fig_TZA_2018_case,
                            nrow = 1
                            )
...
print(fig_TZA_2018_cocir)
```

<!-- ####UGA: 2015 -->

<!-- ```{r co-circulation_UGA_2015, echo=F, message=F, warning=F, fig.height=8, fig.width=16, background="white"} -->

<!-- UGA_neighbors <- data.frame(ISO3 = c("SSD","KEN","TZA","RWA","COD")) -->
<!-- ####UGA_2015 -->

<!-- UGA_2015_case <- read_xlsx("./supp_data/supplementary_appendix_4.xlsx", sheet = "UGA") %>% -->
<!--   dplyr::select(NAME_1,cases15) %>% -->
<!--   rename(case_n = cases15) %>% -->
<!--   mutate(case_cat = factor(case_when(case_n == 0 ~ 1, -->
<!--                                       case_n > 0 & case_n <= 10 ~ 2, -->
...
<!-- UGA_2015_te <- anno_withnode %>% -->
<!--               filter(!stri_isempty(district) & -->
<!--                      te != "sporadic/local outbreak" & -->
<!--                      te != "AFR16" & -->
<!--                      te != "AFR17" -->
<!--                      ) %>% -->
<!--   filter(ISO3 == "UGA" & year == 2015) -->

<!-- UGA_2015_te_sum <- UGA_2015_te %>% group_by(district,te) %>% -->
<!--   summarise(n = n()) %>% ungroup() %>% -->
<!--   mutate(label = paste(te," : ",n,sep = "")) -->

<!-- UGA_2015_te_country <- UGA_2015_te_sum %>% -->
<!--   arrange(district,as.numeric(str_replace(te,"T",""))) %>% -->
<!--   group_by(district) %>% -->
<!--   summarise(te_sum = paste0(te,collapse = ","), -->
<!--             te_label = paste0(label,collapse = ", ")) %>% ungroup() %>% -->
...
<!-- UGA_2015_case_te <- Reduce(function(x,y) {merge(x,y,by = "NAME_1",all.x = T)}, -->
<!--                            list(UGA1,UGA_2015_case,UGA_2015_te_country)) -->


<!-- UGA_2015_neighbor_te <- merge(UGA_neighbors, -->
<!--                               anno_withnode %>% -->
<!--                                 filter(te != "sporadic/local outbreak" & -->
<!--                                        te != "AFR16" & -->
<!--                                        te != "AFR17") %>% -->
<!--                                 filter(ISO3 %in% UGA_neighbors$ISO3 & -->
<!--                                          year == 2015)%>% group_by(ISO3,te) %>% -->
<!--                                 summarise(n = n()) %>% group_by(ISO3) %>% -->
<!--                                 summarise(neighbor_label = -->
<!--                                             paste0(te,collapse = ",")) %>% -->
...
<!-- UGA_2015_neighbor_te$x = c(30,35,30,32,33) -->
<!-- UGA_2015_neighbor_te$y = c(2,0.5,-1.9,4.5,-1.5) -->
<!-- UGA_2015_neighbor_te$size = c('l','l','s','l','l') -->


<!-- fig_UGA_2015_te <- UGA_2015_case_te %>% ggplot()  + -->
<!--                             geom_sf(size=0.6, -->
<!--                                     fill="white" -->
<!--                                     ) + -->
<!--                             geom_sf(size=0.6, -->
<!--                                     aes(fill=te_sum), -->
<!--                                     data=UGA_2015_case_te %>% -->
<!--                                           filter(!is.na(te_sum)) -->
<!--                                     ) + -->
<!--                             geom_sf(data=water_Africa, -->
<!--                                     aes(fill=type) -->
...
<!--                                           y=y -->
<!--                                           ), -->
<!--                                       data=UGA_2015_neighbor_te -->
<!--                                       ) + -->
<!--                             coord_sf(xlim = c(29.5,35.4), -->
<!--                                      ylim = c(-2,4.4)) + -->
<!--                             theme_map() + -->
<!--                             theme(legend.position="right") + -->
<!--                             scale_fill_manual(name="", -->
...
<!-- fig_UGA_2015_case <- UGA_2015_case_te %>% ggplot()   + -->
<!--                             geom_sf(data=UGA_2015_case_te, -->
<!--                                     aes(fill=case_cat), -->
<!--                                     size=1 -->
<!--                                     ) + -->
<!--                             geom_sf(data=water_Africa, -->
...
<!-- fig_UGA_2015_cocir <- ggarrange(fig_UGA_2015_te, -->
<!--                             fig_UGA_2015_case, -->
<!--                             nrow = 1 -->
<!--                             ) -->
...
<!-- print(fig_UGA_2015_cocir) -->
<!-- ``` -->

<!-- ####UGA: 2018 -->

<!-- ```{r co-circulation_UGA_2018, echo=F, message=F, warning=F, fig.height=8, fig.width=16, background="white"} -->

<!-- UGA_neighbors <- data.frame(ISO3 = c("SSD","KEN","TZA","RWA","COD")) -->
<!-- ####UGA_2018 -->

<!-- UGA_2018_case <- read_xlsx("./supp_data/supplementary_appendix_4.xlsx", sheet = "UGA") %>% -->
<!--   dplyr::select(NAME_1,cases18) %>% -->
<!--   rename(case_n = cases18) %>% -->
<!--   mutate(case_cat = factor(case_when(case_n == 0 ~ 1, -->
<!--                                       case_n > 0 & case_n <= 10 ~ 2, -->
...
<!-- UGA_2018_te <- anno_withnode %>% -->
<!--               filter(!stri_isempty(district) & -->
<!--                      te != "sporadic/local outbreak" & -->
<!--                      te != "AFR16" & -->
<!--                      te != "AFR17" -->
<!--                      ) %>% -->
<!--   filter(ISO3 == "UGA" & year == 2018) -->

<!-- UGA_2018_te_sum <- UGA_2018_te %>% group_by(district,te) %>% -->
<!--   summarise(n = n()) %>% ungroup() %>% -->
<!--   mutate(label = paste(te," : ",n,sep = "")) -->

<!-- UGA_2018_te_country <- UGA_2018_te_sum %>% -->
<!--   arrange(district,as.numeric(str_replace(te,"T",""))) %>% -->
<!--   group_by(district) %>% -->
<!--   summarise(te_sum = paste0(te,collapse = ","), -->
<!--             te_label = paste0(label,collapse = ", ")) %>% ungroup() %>% -->
...
<!-- UGA_2018_case_te <- Reduce(function(x,y) {merge(x,y,by = "NAME_1",all.x = T)}, -->
<!--                            list(UGA1,UGA_2018_case,UGA_2018_te_country)) -->


<!-- UGA_2018_neighbor_te <- merge(UGA_neighbors, -->
<!--                               anno_withnode %>% -->
<!--                                 filter(te != "sporadic/local outbreak" & -->
<!--                                        te != "AFR16" & -->
<!--                                        te != "AFR17") %>% -->
...
<!-- UGA_2018_neighbor_te$x = c(30,35,30,32,33) -->
<!-- UGA_2018_neighbor_te$y = c(2,0.5,-1.9,4.5,-1.5) -->
<!-- UGA_2018_neighbor_te$size = c('l','l','s','l','l') -->


<!-- fig_UGA_2018_te <- UGA_2018_case_te %>% ggplot()  + -->
<!--                             geom_sf(size=0.6, -->
<!--                                     fill="white" -->
<!--                                     ) + -->
<!--                             geom_sf(size=0.6, -->
<!--                                     aes(fill=te_sum), -->
<!--                                     data=UGA_2018_case_te %>% -->
<!--                                           filter(!is.na(te_sum)) -->
<!--                                     ) + -->
<!--                             geom_sf(data=water_Africa, -->
<!--                                     aes(fill=type) -->
...
<!-- fig_UGA_2018_case <- UGA_2018_case_te %>% ggplot()   + -->
<!--                             geom_sf(data=UGA_2018_case_te, -->
<!--                                     aes(fill=case_cat), -->
<!--                                     size=1 -->
<!--                                     ) + -->
<!--                             geom_sf(data=water_Africa, -->
...
<!-- fig_UGA_2018_cocir <- ggarrange(fig_UGA_2018_te, -->
<!--                             fig_UGA_2018_case, -->
<!--                             nrow = 1 -->
<!--                             ) -->
...
<!-- print(fig_UGA_2018_cocir) -->
<!-- ``` -->


<!-- ####ZMB: 2016 -->

<!-- ```{r co-circulation_ZMB_2016, echo=F, message=F, warning=F, fig.height=8, fig.width=16, background="white"} -->

<!-- ZMB_neighbors <- data.frame(ISO3 = c("AGO","TZA","MWI","COD", -->
<!--                                      "MOZ","ZWE","BWA","NAM")) -->
<!-- ####ZMB_2016 -->

<!-- ZMB_2016_case <- read_xlsx("./supp_data/supplementary_appendix_4.xlsx", sheet = "ZMB") %>% -->
<!--   dplyr::select(NAME_1,cases16) %>% -->
<!--   rename(case_n = cases16) %>% -->
<!--   mutate(case_cat = factor(case_when(case_n == 0 ~ 1, -->
<!--                                       case_n > 0 & case_n <= 10 ~ 2, -->
...
<!-- ZMB_2016_te <- anno_withnode %>% -->
<!--               filter(!stri_isempty(district) & -->
<!--                      te != "sporadic/local outbreak" & -->
<!--                      te != "AFR16" & -->
<!--                      te != "AFR17" -->
<!--                      ) %>% -->
<!--   filter(ISO3 == "ZMB" & year == 2016) -->

<!-- ZMB_2016_te_sum <- ZMB_2016_te %>% group_by(district,te) %>% -->
<!--   summarise(n = n()) %>% ungroup() %>% -->
<!--   mutate(label = paste(te," : ",n,sep = "")) -->

<!-- ZMB_2016_te_country <- ZMB_2016_te_sum %>% -->
<!--   arrange(district,as.numeric(str_replace(te,"T",""))) %>% -->
<!--   group_by(district) %>% -->
<!--   summarise(te_sum = paste0(te,collapse = ","), -->
<!--             te_label = paste0(label,collapse = ", ")) %>% ungroup() %>% -->
...
<!-- ZMB_2016_case_te <- Reduce(function(x,y) {merge(x,y,by = "NAME_1",all.x = T)}, -->
<!--                            list(ZMB1,ZMB_2016_case,ZMB_2016_te_country)) -->


<!-- ZMB_2016_neighbor_te <- merge(ZMB_neighbors, -->
<!--                               anno_withnode %>% -->
<!--                                 filter(te != "sporadic/local outbreak" & -->
<!--                                        te != "AFR16" & -->
<!--                                        te != "AFR17") %>% -->
<!--                                 filter(ISO3 %in% ZMB_neighbors$ISO3 & -->
<!--                                          year == 2016)%>% group_by(ISO3,te) %>% -->
<!--                                 summarise(n = n()) %>% group_by(ISO3) %>% -->
<!--                                 summarise(neighbor_label = -->
<!--                                             paste0(te,collapse = ",")) %>% -->
<!--                                 ungroup() %>% mutate(neighbor_label = -->
...
<!-- ZMB_2016_neighbor_te$x = c(20,24,26,33,33.7,20,32,30) -->
<!-- ZMB_2016_neighbor_te$y = c(-12,-19,-10,-15,-13,-19,-7.9,-18) -->
<!-- ZMB_2016_neighbor_te$size = c('l','l','l','s','s','s','l','l') -->

<!-- fig_ZMB_2016_te <- ZMB_2016_case_te %>% ggplot()  + -->
<!--                             geom_sf(size=0.6, -->
<!--                                     fill="white" -->
<!--                                     ) + -->
<!--                             geom_sf(size=0.6, -->
<!--                                     aes(fill=te_sum), -->
<!--                                     data=ZMB_2016_case_te %>% -->
<!--                                           filter(!is.na(te_sum)) -->
<!--                                     ) + -->
<!--                             geom_sf(data=water_Africa, -->
<!--                                     aes(fill=type) -->
...
<!--                                           y=y -->
<!--                                           ), -->
<!--                                       data=ZMB_2016_neighbor_te -->
<!--                                       ) + -->
<!--                             coord_sf(xlim = c(19.5,33.8), -->
<!--                                      ylim = c(-20,-7)) + -->
<!--                             theme_map() + -->
<!--                             theme(legend.position="right") + -->
<!--                             scale_fill_manual(name="", -->
...
<!-- fig_ZMB_2016_case <- ZMB_2016_case_te %>% ggplot()   + -->
<!--                             geom_sf(data=ZMB_2016_case_te, -->
<!--                                     aes(fill=case_cat), -->
<!--                                     size=1 -->
<!--                                     ) + -->
<!--                             geom_sf(data=water_Africa, -->
...
<!-- fig_ZMB_2016_cocir <- ggarrange(fig_ZMB_2016_te, -->
<!--                             fig_ZMB_2016_case, -->
<!--                             nrow = 1 -->
<!--                             ) -->
