...
## unzip the water shape file
unzip("maps/8ark3lcpfw_GLWD_level1.zip",
      exdir="maps")


##set up the coordinate referece system for the water shape file
...
##get water shape file for africa
...
# read Appendix 2- Tab 2: Sample metadata and sequencing metrics
## filter out genomes not meeting criteria
new_seq <- 
  read_xlsx("supp_data/Appendix2.xlsx",
            sheet="Tab2-Metrics",
            skip=2) %>% 
  filter(meeting_criteria)
...
# read Appendix 2- Tab 1: Background sequence accessions and metadata
annotation_raw <- read_xlsx("supp_data/Appendix2.xlsx",
            sheet="Tab1-Background",
            skip=2)
...
# read Appendix 3 - TAB 1: Cholera case counts by country and year
case_data <- read_xlsx("supp_data/Appendix3.xlsx",
                       sheet="Tab1-Country",
                       skip=2) %>% 
             rename(year=Year) %>%
...
# read Appendix 2 - TAB 3: Median depth distribution by sample type from sub-sampling analysis
subsample_bysample_depth <-
  read_xlsx("supp_data/Appendix2.xlsx",
            sheet="Tab3-DepthByType",
            skip=2) 
...
# read Appendix 2 - TAB 4: Median depth distribution by nucleotide position (4000-nt aggregated) after downsampling
subsample_pos_cov_binned <- 
  read_xlsx("supp_data/Appendix2.xlsx",
            sheet="Tab4-DepthByPos",
            skip=2) 
...
# read ML tree
tree_1488 <- read.tree("supp_data/Appendix4.tree")
...
## Appendix 2 - TAB 5: Raw data file for posterior distribution of tMRCA of the AFR13 sub-lineage
T13_tMRCA <- read_xlsx("supp_data/Appendix2.xlsx",
                       sheet="Tab5-tMRCA",
                       skip=2) %>%
...
# Appendix 3 - TAB 2: Cholera case counts in Kenya by sub-country location and year
## case numbers in Kenya - 2009
KEN_2009_case <- read_xlsx("supp_data/Appendix3.xlsx",
                           sheet="Tab2-Kenya",
                           skip=2) %>%
...
# Appendix 3 - TAB 2: Cholera case counts in Kenya by sub-country location and year
## case numbers in Kenya - 2010
KEN_2010_case <- read_xlsx("supp_data/Appendix3.xlsx",
                           sheet="Tab2-Kenya",
                           skip=2) %>%
...
# Appendix 3 - TAB 4: Cholera case counts in Malawi by sub-country location and year
## case numbers in Malawi - 2015
MWI_2015_case <- read_xlsx("supp_data/Appendix3.xlsx",
                           sheet="Tab4-Malawi",
                           skip=2) %>%
...
# Appendix 3 - TAB 4: Cholera case counts in Malawi by sub-country location and year
## case numbers in Malawi - 2016
MWI_2016_case <- read_xlsx("supp_data/Appendix3.xlsx",
                           sheet="Tab4-Malawi",
                           skip=2) %>%
...
# Appendix 5 - TAB 1: Root-to-tip distance and residual from best-fit line for all sequences in tree
vc_1488_rtt <- 
  read_xlsx("supp_data/Appendix5.xlsx",
             sheet = "Tab1-ALL",
             skip=2)
...
suppfig1_panelA <- vc_1488_rtt %>% ggplot() + 
                          ...
                          coord_cartesian(xlim=c(vc_1488_rtt_xintercept - 5,
                                          ...
                                          ylim=c(0, max(vc_1488_rtt$distance + 0.005)))+
...
# Appendix 5 - TAB 2: Root-to-tip distance and residual from best-fit line for all Wave 3 sequences
vc_wave3_rtt <- 
  read_xlsx("supp_data/Appendix5.xlsx",
             sheet = "Tab2-Wave3",
             skip=2)
...
suppfig1_panelB <- vc_wave3_rtt %>% ggplot() + 
                        ...
                        stat_poly_eq(aes(label = after_stat(adj.rr.label)))+ 
                          coord_cartesian(xlim=c(vc_wave3_rtt_xintercept - 5,
                                          ...
                                          ylim=c(0, max(vc_wave3_rtt$distance)+0.0005))+
...
### SuppFigure2: Wave 3 maximum clade credibility tree. 

### SuppFigure3A: The root-to-tip regression visualization of 186 genomes belonging to AFR13 clade
```{r supp_fig3A, echo=F, message=F, warning=F, fig.height=8, fig.width=8, background="white"}


# Appendix 5 - TAB 3: Root-to-tip distance and residual from best-fit line for AFR13 sequences
afr13_rtt <- read_xlsx("supp_data/Appendix5.xlsx",
                       sheet = "Tab3-AFR13",
                       skip=2) %>% 
  mutate(tip=recode(tip,
                    "CP003069.1_CP003070.1" = "CP003069.1/CP003070.1"),
...
afr13_rtt_withanno <- 
  merge(afr13_rtt,
        anno_withnode %>% 
          dplyr::select(taxa, ISO3,year, te, status, ref, accession),
        by.x="tip", by.y="taxa",all.x=T
        ) %>%
  mutate(group = 
           factor(
             case_when(!((year <= 2013 & ISO3 == "KEN") | 
                         (str_detect(accession,"^SAMN191"))
                         ) ~ 0,
                        (year <= 2013 & ISO3 == "KEN") & 
                        (!str_detect(accession,"^SAMN191")) ~ 1 ,
                       !((year <= 2013 & ISO3 == "KEN")) & 
                         (str_detect(accession,"^SAMN191")) ~ 2),
             labels=c("other AFR13 or background genomes",
                      "10 Kenya isolates\ncollected prior to 2013 in our study",
                      "Published Tanzania isolates\nassembled from contigs")))


## fit the regression model
afr13_rtt_model <- lm(distance ~ date, data=afr13_rtt_withanno)

## get the x intercept
afr13_rtt_xintercept <- -coef(afr13_rtt_model)[1]/coef(afr13_rtt_model)[2]

## Supp Fig3A
suppfig3_panelA <- afr13_rtt_withanno %>% ggplot() + 
  aes(x=date, y=distance) +
  geom_point(aes(color=group)) +
  geom_abline(intercept=coef(afr13_rtt_model)[1],
              slope=coef(afr13_rtt_model)[2],
              color="black")+
  ylab("root-to-tip divergence") + 
  xlab("time") +
  scale_color_manual(name="",
  ...
  stat_poly_eq(aes(label = after_stat(adj.rr.label))) + 
  coord_cartesian(xlim=c(afr13_rtt_xintercept - 3,
                         max(afr13_rtt_withanno$date) + 3),
                  ylim=c(0, max(afr13_rtt_withanno$distance)+0.0005))+
  scale_x_continuous(breaks = c(afr13_rtt_xintercept, 2010,2015,2020),
                     labels=c(sprintf("%.2f",
                                      afr13_rtt_xintercept),2010,2015,2020)) +
  scale_y_continuous(expand=c(0,0))+
  theme_bw(base_size=18) +
  theme(panel.border=element_blank(),
        panel.grid.major=element_blank(),
...
print(suppfig3_panelA)


## compare of RMSE
afr13_rtt_withanno$model_predict <- predict(afr13_rtt_model)

## RMSE all genomes afr13
rmse_afr13_all <- sqrt(mean((afr13_rtt_withanno$model_predict - 
                             afr13_rtt_withanno$distance)^2))

## RMSE of genomes minus kenya 10
rmse_afr13_all_excludekenya10 <- sqrt(mean((afr13_rtt_withanno %>% 
                               filter(group != "10 Kenya isolates\ncollected prior to 2013 in our study") %>%
                               pull(model_predict)- 
                             afr13_rtt_withanno  %>% 
                               filter(group != "10 Kenya isolates\ncollected prior to 2013 in our study") %>%
                               pull(distance))^2))
...
## RMSE of kenya 10
rmse_afr13_kenya10 <- sqrt(mean((afr13_rtt_withanno %>% 
                               filter(group == "10 Kenya isolates\ncollected prior to 2013 in our study") %>%
                               pull(model_predict)- 
                             afr13_rtt_withanno  %>% 
                               filter(group == "10 Kenya isolates\ncollected prior to 2013 in our study") %>%
...
## RMSE of genomes sequenced in our study minus kenya 10
rmse_afr13_sequencedinstudy_excludekenya10 <- sqrt(mean((afr13_rtt_withanno %>% 
                               filter(group != "10 Kenya isolates\ncollected prior to 2013 in our study" &
                                      status != "Published genomes") %>%
                               pull(model_predict)- 
                             afr13_rtt_withanno  %>% 
                               filter(group != "10 Kenya isolates\ncollected prior to 2013 in our study" &
                                      status != "Published genomes") %>%
...
### SuppFigure3B: The root-to-tip regression visualization of AFR13 clade without the Tanzania assembled contigs
```{r supp_fig3B, echo=F, message=F, warning=F, fig.height=8, fig.width=10, background="white"}


afr13_rtt_withoutcontig <- 
  afr13_rtt_withanno %>% 
  filter(group != "Published Tanzania isolates\nassembled from contigs") %>%
  mutate(group = factor(case_when(group == "other AFR13 or background genomes" &
                           status == "Published genomes" ~ 0,
                           group == "other AFR13 or background genomes" &
                           status != "Published genomes" ~ 1,
                           group == "10 Kenya isolates\ncollected prior to 2013 in our study" ~ 2),
                        labels=c("other AFR13 or background published genomes",
                                 "other AFR13 or background genomes sequenced in this paper",
                                 "10 Kenya isolates\ncollected prior to 2013 in our study")
                        )
         )
...
## fit the regression model
afr13_rtt_withoutcontig_model <- 
  lm(distance ~ date, data=afr13_rtt_withoutcontig)

## get the x intercept
afr13_rtt_withoutcontig_xintercept <- 
  -coef(afr13_rtt_withoutcontig_model)[1]/coef(afr13_rtt_withoutcontig_model)[2]


suppfig3_panelB <- afr13_rtt_withoutcontig %>% ggplot() + 
  aes(x=date, y=distance) +
  geom_point(aes(color=group)) +
  geom_abline(intercept=coef(afr13_rtt_withoutcontig_model)[1],
              slope=coef(afr13_rtt_withoutcontig_model)[2],
              color="black")+
  ylab("root-to-tip divergence") + 
  xlab("time") +
  scale_color_manual(name="",
  ...
  stat_poly_eq(aes(label = after_stat(adj.rr.label))) + 
  coord_cartesian(xlim=c(afr13_rtt_withoutcontig_xintercept - 3,
                         max(afr13_rtt_withoutcontig$date) + 3),
                  ylim=c(0, max(afr13_rtt_withoutcontig$distance)+0.0002))+
  scale_x_continuous(breaks = c(afr13_rtt_withoutcontig_xintercept,
                                2010,2015,2020),
                     labels=c(sprintf("%.2f",
                                      afr13_rtt_withoutcontig_xintercept),
                              2010,2015,2020)) +
...
print(suppfig3_panelB)

## compare of RMSE
afr13_rtt_withoutcontig$model_predict <- predict(afr13_rtt_withoutcontig_model)

## RMSE all genomes afr13
rmse_withoutcontig_afr13_all <- sqrt(mean((afr13_rtt_withoutcontig$model_predict - 
                                           afr13_rtt_withoutcontig$distance)^2))

## RMSE of genomes minus kenya 10
rmse_withoutcontig_afr13_all_excludekenya10 <- sqrt(mean((afr13_rtt_withoutcontig %>% 
                               filter(group != "10 Kenya isolates\ncollected prior to 2013 in our study") %>%
                               pull(model_predict)- 
                             afr13_rtt_withoutcontig  %>% 
                               filter(group != "10 Kenya isolates\ncollected prior to 2013 in our study") %>%
...
## RMSE of kenya 10
rmse_withoutcontig_afr13_kenya10 <- sqrt(mean((afr13_rtt_withoutcontig %>% 
                               filter(group == "10 Kenya isolates\ncollected prior to 2013 in our study") %>%
                               pull(model_predict)- 
                             afr13_rtt_withoutcontig  %>% 
                               filter(group == "10 Kenya isolates\ncollected prior to 2013 in our study") %>%
...
## RMSE of genomes sequenced in our study minus kenya 10
rmse_withoutcontig_afr13_sequencedinstudy_excludekenya10 <- sqrt(mean((afr13_rtt_withoutcontig %>% 
                               filter(group != "10 Kenya isolates\ncollected prior to 2013 in our study" &
                                      status != "Published genomes") %>%
                               pull(model_predict)- 
                             afr13_rtt_withoutcontig  %>% 
